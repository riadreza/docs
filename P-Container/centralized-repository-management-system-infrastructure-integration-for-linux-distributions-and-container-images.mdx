...
```mermaid
graph TD
    A[Developers / CI/CD] -->|Push/Pull Artifacts| B(Nexus Repository Manager)
    B -->|Proxy Requests| C[External Repositories <br>(Docker Hub, Ubuntu, etc.)]
    B -->|Serve Cached/Hosted| A
    
    subgraph "Nexus Hosts"
        B
        direction LR
        B1[Container Images <br> (docker)]
        B2[Linux Packages <br> (apt/yum)]
    end
```
...
networks:
  traefik:
    external: true
```
b. Create the necessary directories on your NFS share: +Create the necessary directories on your NFS share:
```bash
mkdir -p /mnt/nfs-share/nexus/data
mkdir -p /mnt/nfs-share/nexus/storage
```
c. Give file system permissions to the nexus user (UID 200): +Give file system permissions to the nexus user (UID 200), which runs the Nexus process inside the container:
```bash
...
``` 
d. Deploy the Nexus Repository Manager using Docker Compose: +Deploy the Nexus Repository Manager as a Docker stack:
```bash
...
1. If you are using HTTPS for your Nexus Repository Manager, ensure that the SSL certificate is trusted by your clients. You may need to download the certificate from your Nexus server and add it to the trusted certificate store on your client machines.  
```bash
openssl s_client -showcerts -connect repo.palonet.org:443 </dev/null 2>/dev/null | openssl x509 -outform PEM > /tmp/nexus.crt
```
2. Install the certificate:
- For RPM-based systems:
```bash
sudo cp /tmp/nexus.crt /usr/local/share/ca-certificates/nexus.crt
sudo update-ca-certificates
```
- For DEB-based systems:
```bash
sudo cp /tmp/nexus.crt /etc/pki/ca-trust
